	...
	\begin{table}[ht]
		...
		\makebox[\linewidth]{
			\begin{tabular}{|p{2cm}|p{3cm}|p{3cm}|p{3cm}|p{3cm}|p{2cm}|p{3cm}|p{1cm}|p{1cm}|}
				...
				Graph Creation & Graph cannot be generated  & Not enough past information from user  & Cannot output a graph for user to see & There is no information available in users past data  & & Display error message stating there is not enough data to create the graph & & H6-1 \\ \hline
	...
	\begin{table}[ht]
		...
		\makebox[\linewidth]{
			\begin{tabular}{|p{2cm}|p{3cm}|p{3cm}|p{3cm}|p{3cm}|p{2cm}|p{3cm}|p{1cm}|p{1cm}|}
				...
				\multicolumn{9}{|l|}{Phase/Mode: System Requirements}\\\hline
				\textbf{Design Function} & \textbf{Failure Modes} & \textbf{Effects of Failure} & \textbf{Causes of Failure} & \textbf{Detection} & \textbf{Risk Priority Number} & \textbf{Recommended Actions} & \textbf{SR} & \textbf{Ref} \\ \hline
				API Calls & API call fails unexpectedly & a. Internet connection error & System will not return nutritional data for a food item & The API response will be verified by the system. The system will detect if the response is an error & & a. Display error message detailing a system error due to poor internet connection& SR2 & \multirow{2}{*} {HX-1} \\ 
				& & b. Too many requests sent to API causing throttling limit to be reached & & & & b. Display error message detailing a system error due to too many requests being sent & & \\ \cline{2-9}
				& API does not contain nutrition facts for a food item & Food item is not found in API database & Same as HX-1 & The API response will be verified by the system and will detect if the response is empty & & Display error message detailing that the food data could not be fetched & SR2 & HX-2\\ \hline
				Image Identification Model & Food from image is incorrectly identified & a. Poor image quality & System will process the incorrectly identified food item & User will file a report to the dev team with the image that was incorrectly identified & & User should upload an alternate image of the food & SR2, SR3 & \multirow{3}{*} {HY-1} \\ 
				& & b. Food identification system error & & & & & & \\
				& & c. ML system biases & & & & & & \\ \cline{2-9}
				& No food is identified in the image & Same as HY-1 & No food item will be identified, the system will not be able to proceed & The food identification ML process will return an error to the system & & Display error message detailing that a food item was not detected & SR2, SR3 & HY-2\\ \hline
