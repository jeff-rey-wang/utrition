...
def startModel(flag, PA):
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for i in range(
            len(data_sets["images_test"])
        ):  # for each test image, display the accuracy at which the model correctly predicts the images label
            test_accuracy = sess.run(
                accuracy,
                feed_dict={
                    images_placeholder: data_sets["images_test"][[i]],
                    labels_placeholder: data_sets["labels_test"][[i]],
                },
            )
            if (
                test_accuracy > 0
            ):  # if match accuracy > 0, display the class of identified object (this should only be true once per image)
                print("-" * 30)
